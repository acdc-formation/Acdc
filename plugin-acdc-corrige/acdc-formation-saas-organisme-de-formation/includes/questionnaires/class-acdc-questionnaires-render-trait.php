...
public function render_quality_compliance_shortcode( $atts = array() ) {
  ...
  private function render_survey_dashboard_stats( $survey_type, $source_id = 0 ) {
    ...
    ?>
    <div class="acdc-grid-4cols acdc-mb-18" style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:18px;">
    ...
    <div class="acdc-grid-3cols acdc-mb-18" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px;">
      ...
      <form method="get" action="<?php echo esc_url( is_admin() ? admin_url( 'admin.php' ) : $base_url ); ?>" class="acdc-filters-bar">
        ...
        <select name="session_status">
          ...
          <?php foreach ( array( 'brouillon','planifiee','envoyee','prete','ouverte','en_cours','en_pause','terminee','expiree','annulee','archivee' ) as $status ) : ?>
            <option value="<?php echo esc_attr( $status ); ?>" <?php selected( $filter_status, $status ); ?>><?php echo esc_html( ucfirst( str_replace( '_', ' ', $status ) ) ); ?></option>
          <?php endforeach; ?>
        </select>
    ...
    <?php if ( in_array( $action, array( 'new', 'edit', 'view', 'animate' ), true ) ) : ?>
      ...
      <div class="acdc-panel acdc-mb-18"><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="acdc-form">
        ...
        <div class="acdc-grid-2cols acdc-mb-18">
          ...
          <p><label>Statut</label><select name="questionnaire_session[status]" <?php echo 'view' === $action ? 'disabled' : ''; ?>><?php foreach ( array( 'brouillon','planifiee','envoyee','prete','ouverte','en_cours','en_pause','terminee','expiree','annulee','archivee' ) as $status ) : ?><option value="<?php echo esc_attr( $status ); ?>" <?php selected( $session ? $session->status : $questionnaire_defaults['new_session_status_default'], $status ); ?>><?php echo esc_html( ucfirst( str_replace( '_', ' ', $status ) ) ); ?></option><?php endforeach; ?></select></p>
        ...
        <?php if ( $session ) : $joined_participants = $this->get_questionnaire_session_participants( $session->id ); ?>
          <div class="acdc-panel acdc-panel-block acdc-mb-18"><h4 style="margin-top:0;">Participants déjà connectés</h4>
            <div class="acdc-table-wrap"><table class="acdc-table"><thead><tr><th>Pseudo</th><th>Apprenant</th><th>Statut</th><th>Score</th></tr></thead><tbody><?php foreach ( $joined_participants as $participant ) : $participant_name = trim( ( $participant->first_name ?? '' ) . ' ' . ( $participant->last_name ?? '' ) ); ?><tr><td><?php echo esc_html( $participant->pseudo ); ?></td><td><?php echo esc_html( $participant_name ? $participant_name : '—' ); ?></td><td><?php echo esc_html( ucfirst( str_replace( '_', ' ', $participant->participant_status ) ) ); ?></td><td><?php echo esc_html( $this->get_questionnaire_session_participant_score( $session->id, $participant->id ) ); ?></td></tr><?php endforeach; if ( empty( $joined_participants ) ) : ?><tr><td colspan="4">Aucun participant connecté pour le moment.</td></tr><?php endif; ?></tbody></table></div>
          </div>
        <?php endif; ?>
