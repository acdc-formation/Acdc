...
trait ACDC_Documents_Billing_Render_Trait {
  ...
  private function render_front_quote_view( $scope, $quote_id ) {
    ...
    <div class="acdc-panel acdc-profile-section"><h3>Devis</h3><div class="acdc-details-grid"><div>Télécharger</div><div><a href="<?php echo esc_url( $download_url ); ?>" class="acdc-file-chip">📄 Devis <?php echo esc_html( $row['number'] ); ?>.html</a></div><div>Statut</div><div><span class="acdc-status-badge"><?php echo esc_html( $row['status'] ); ?></span></div><div>Préfixe</div><div><?php echo esc_html( $row['prefix'] ); ?></div><div>Numéro</div><div><?php echo esc_html( $row['num'] ); ?></div><div>Date d'émission</div><div><?php echo esc_html( $row['emission_date'] ); ?></div><div>Date d'expiration</div><div><?php echo esc_html( $row['expiration_date'] ); ?></div><div>Désignation</div><div><a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener">Afficher le contenu</a></div><div>Quantité</div><div><?php echo esc_html( $row['quantity'] ); ?></div><div>Tarif de l'action de formation (€ HT)</div><div><?php echo esc_html( $row['tarif_ht'] ); ?></div><div>Taux de TVA (%)</div><div><?php echo esc_html( isset( $row['vat_rate'] ) && '' !== (string) $row['vat_rate'] ? (string) $row['vat_rate'] : '0,00' ); ?> %</div><div>Frais de transport</div><div>⛔</div><div>Frais de restauration et / ou hébergement</div><div>⛔</div><div>Lignes supplémentaires</div><div>—</div><div>Tarif de l'action de formation (€ TTC)</div><div><?php echo esc_html( $row['tarif_ttc'] ); ?></div><div>Adresse d'émission</div><div><a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener">Afficher le contenu</a></div><div>Méthodes de paiement</div><div><a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener">Afficher le contenu</a></div><div>Commentaire (n'apparaît pas sur le devis)</div><div>—</div><div>Pièce jointe</div><div>—</div><div>Programme de formation</div><div>⛔</div><div>CGV</div><div>⛔</div></div></div>
