...
trait ACDC_Documents_Billing_Render_Trait {
        ...
        <table class="acdc-table acdc-quotes-table" data-acdc-table-id="quotes-list">
          ...
          <?php if ( ! empty( $rows ) ) : ?>
            <?php foreach ( $rows as $row ) :
              ...
              $pdf_url      = $this->secure_admin_post_url( 'acdc_download_quote_pdf', array( 'quote_id' => (int) $row['id'] ), 'acdc_download_quote_pdf_' . (int) $row['id'] );
              ...
              $menu_items_top = array(
                ...
                'Version imprimable (aperçu)' => $download_url,
            ...
            <tr>
              ...
              <td><span class="acdc-status-badge"><?php echo esc_html( $row['status_label'] ?? ( $this->get_quote_status_labels()[ $row['status'] ?? '' ] ?? ucfirst( (string) ( $row['status'] ?? 'Brouillon' ) ) ) ); ?></span></td>
  ...
  private function render_front_quote_view( $scope, $quote_id ) {
    ...
    <div style="display:flex;justify-content:flex-end;gap:10px;margin-bottom:14px;">
      ...
      <a href="<?php echo esc_url( $back_url ); ?>" class="acdc-button acdc-button-soft">&larr; Retour</a>
    </div>
    ...
    <div class="acdc-panel acdc-profile-section"><h3>Devis</h3><div class="acdc-details-grid"><div>Télécharger</div><div><a href="<?php echo esc_url( $pdf_url ); ?>" class="acdc-file-chip">📄 Devis <?php echo esc_html( $row['number'] ); ?>.pdf</a> <a href="<?php echo esc_url( $download_url ); ?>" class="acdc-file-chip" target="_blank" rel="noopener">🖨️ Version imprimable (aper&#231;u)</a></div><div>Statut</div><div><span class="acdc-status-badge"><?php echo esc_html( $row['status_label'] ?? ( $this->get_quote_status_labels()[ $row['status'] ?? '' ] ?? ucfirst( (string) $row['status'] ) ) ); ?></span></div><div>Préfixe</div><div><?php echo esc_html( $row['prefix'] ); ?></div><div>Numéro</div><div><?php echo esc_html( $row['num'] ); ?></div><div>Date d'émission</div><div><?php echo esc_html( $row['emission_date'] ); ?></div><div>Date d'expiration</div><div><?php echo esc_html( $row['expiration_date'] ); ?></div><div>Désignation</div><div><a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener">Afficher le contenu</a></div><div>Quantité</div><div><?php echo esc_html( $row['quantity'] ); ?></div><div>Tarif de l'action de formation (€ HT)</div><div><?php echo esc_html( $row['tarif_ht'] ); ?></div><div>Taux de TVA (%)</div><div><?php echo esc_html( isset( $row['vat_rate'] ) && '' !== (string) $row['vat_rate'] ? (string) $row['vat_rate'] : '0,00' ); ?> %</div><div>Frais de transport</div><div>⛔</div><div>Frais de restauration et / ou hébergement</div><div>⛔</div><div>Lignes supplémentaires</div><div>—</div><div>Tarif de l'action de formation (€ TTC)</div><div><?php echo esc_html( $row['tarif_ttc'] ); ?></div><div>Adresse d'émission</div><div><a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener">Afficher le contenu</a></div><div>Méthodes de paiement</div><div><a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener">Afficher le contenu</a></div><div>Commentaire (n'apparaît pas sur le devis)</div><div>—</div><div>Pièce jointe</div><div>—</div><div>Programme de formation</div><div>⛔</div><div>CGV</div><div>⛔</div></div></div>
  ...
  private function render_front_quote_preview( $scope, $quote_id ) {
    ...
    echo '<div class="acdc-mb-18"><a class="acdc-button acdc-button-soft" href="' . esc_url( $back_url ) . '" style="min-width:44px;padding:10px 14px;">&larr;</a></div>';
  ...
  private function render_front_invoice_view( $scope, $invoice_id ) {
    ...
    $facturx_pdf_url = $is_demo ? '' : $this->secure_admin_post_url( 'acdc_download_invoice_facturx_pdf', array( 'invoice_id' => (int) $row['id'] ), 'acdc_download_invoice_facturx_pdf_' . (int) $row['id'] );
    ...
    <div class="acdc-panel acdc-profile-section"><h3>Facture <?php echo esc_html( $row['number'] ); ?></h3><div class="acdc-details-grid">
      ...
      <?php if ( $facturx_url ) : ?><div>Facture électronique</div><div><a href="<?php echo esc_url( $facturx_url ); ?>" class="acdc-file-chip" title="XML Factur-X (profil MINIMUM) à embarquer dans le PDF/A-3">🧾 Factur-X (XML)</a><?php if ( $facturx_pdf_url ) : ?> <a href="<?php echo esc_url( $facturx_pdf_url ); ?>" class="acdc-file-chip" title="Facture PDF/A-3 avec le XML Factur-X (profil MINIMUM) embarqué">🧾 Factur-X (PDF/A-3)</a><?php endif; ?></div><?php endif; ?>
